<template>
  <div class="page-container">
    ...
    <div class="container mx-auto px-6 py-12 max-w-5xl">
      <button @click="$router.push('/companions')" class="btn-ghost mb-8">
      ...
      <template v-else-if="item">
        ...
        <div class="glass-panel p-8 mb-8">
          <div class="flex flex-col md:flex-row gap-6">
            ...
            <div class="flex-1">
              <h1 class="text-2xl md:text-3xl font-bold mb-2" style="color: var(--color-text);">{{ item.title }}</h1>
              <p v-if="item.description" class="text-sub mb-4">{{ item.description }}</p>
        ...
        <div class="flex gap-2 mb-6">
          <button @click="activeTab = 'articles'" class="px-5 py-2 rounded-xl text-sm font-medium transition-all"
            :style="activeTab === 'articles' ? { background: 'var(--color-primary)', color: '#fff' } : { background: 'var(--color-surface)', color: 'var(--color-text-dim)' }">
            <Icon name="solar:document-text-bold-duotone" size="sm" class="mr-1" />文章 {{ allArticles.length }}
          </button>
          <button @click="activeTab = 'gallery'" class="px-5 py-2 rounded-xl text-sm font-medium transition-all"
            :style="activeTab === 'gallery' ? { background: 'var(--color-primary)', color: '#fff' } : { background: 'var(--color-surface)', color: 'var(--color-text-dim)' }">
            <Icon name="solar:gallery-wide-bold-duotone" size="sm" class="mr-1" />图廊 {{ galleryImages.length }}
          </button>
        </div>
        ...
        <div v-if="activeTab === 'articles'">
          <main class="max-w-3xl mx-auto">
            <div v-if="currentArticle" class="article-body">
              ...
              <div class="article-content prose dark:prose-invert" :style="{ fontSize: fontSizePx + 'px', lineHeight: lineHeight }">
            ...
            <div v-else class="glass-card p-12 text-center">
              ...
              <p class="text-sub text-sm">从左侧目录中选择一篇开始阅读</p>
            </div>
          </main>
        </div>

        <!-- Gallery Tab -->
        <OCGallery v-if="activeTab === 'gallery'" :images="galleryImages" />

        <!-- Mobile TOC Side Tab -->
        <template v-if="activeTab === 'articles' && allArticles.length > 0">
          <button
              ...
              <div v-if="showMobileToc" class="lg:hidden fixed inset-0 z-[70] flex">
                ...
                <div class="w-[75vw] max-w-[320px] flex flex-col shadow-2xl" style="background: var(--color-bg-deep); border-left: 1px solid var(--color-border-subtle);">
                  ...
                  <nav class="flex-1 overflow-y-auto p-3 space-y-0.5">
                    <button v-for="art in allArticles" :key="art.id" @click="loadArticle(art.id); showMobileToc = false"
                  ...
                  <div class="flex items-center gap-2 p-3 border-t flex-shrink-0" style="border-color: var(--color-border-subtle);">
                    <button v-if="prevArticle" @click="loadArticle(prevArticle.id); showMobileToc = false"
                    ...
                    <button v-if="nextArticle" @click="loadArticle(nextArticle.id); showMobileToc = false"
                      class="flex-1 flex items-center justify-center gap-1 px-3 py-2 rounded-lg text-xs font-medium transition-colors hover:bg-white/5"
                      style="color: var(--color-text-dim);">
      ...
      <div v-else class="glass-card p-12 text-center">
        <h3 class="text-xl font-semibold mb-2" style="color: var(--color-text);">OC 不存在</h3>
        <button @click="$router.push('/companions')" class="btn-ghost mt-4">返回 OC 社区</button>
      </div>
    </div>
  </div>
</template>
...
const activeTab = ref<'articles' | 'gallery'>('articles')
const progressPercent = ref(0)
const showMobileToc = ref(false)

const fontSize = computed(() => readerState.fontSize)
const fontSizePx = computed(() => readerState.fontSize === 'lg' ? 20 : readerState.fontSize === 'xl' ? 25 : 16)
const lineHeight = computed(() => readerState.fontSize === 'lg' ? 2.0 : readerState.fontSize === 'xl' ? 2.2 : 1.85)
...
const formatDate = (d: string) => { if (!d) return ''; return new Date(d).toLocaleDateString('zh-CN', { year: 'numeric', month: 'long', day: 'numeric' }) }

async function loadArticle(articleId: number) {
  try {
    const res = await itemAPI.getArticle(Number(route.params.id), articleId)
    currentArticle.value = res.article
    activeArticleId.value = articleId
    readerState.activeId = articleId
...
function downloadArticle() {
  ...
  const a = document.createElement('a'); a.href = url; a.download = `${currentArticle.value.title}.txt`; a.click()
...
onMounted(async () => {
  try {
    ...
    const [detailRes, tocRes, imgRes] = await Promise.all([
      itemAPI.getById(id), itemAPI.getTOC(id),
      fetch(`/api/oc-images/item/${id}`).then(r => r.json())
    ])
    item.value = detailRes.item; galleryImages.value = imgRes.images || []
    const articles: any[] = []
    ...
    readerState.articles = articles.map(a => ({ id: a.id, title: a.title, word_count: a.word_count }))
...
<style scoped>
.article-body { max-width: 48rem; margin: 0 auto; background: var(--color-surface); border: 1px solid var(--color-border-subtle); border-radius: 1.5rem; padding: 2.5rem; backdrop-filter: blur(16px); }
.article-title { font-size: 1.75rem; font-weight: 700; line-height: 1.3; margin-bottom: 0.75rem; color: var(--color-text); }
.article-meta { display: flex; align-items: center; gap: 0.5rem; font-size: 0.8125rem; color: var(--color-text-caption); margin-bottom: 2rem; padding-bottom: 1.25rem; border-bottom: 1px solid var(--color-border-subtle); }
...
.article-nav-btn { flex: 1; padding: 0.75rem 1rem; border-radius: 0.75rem; transition: all 0.2s; background: var(--color-surface); border: 1px solid var(--color-border-subtle); }
...
@media (max-width: 1024px) { .article-body { padding: 1.5rem; border-radius: 1rem; } .article-title { font-size: 1.35rem; } }
</style>
